...
public class ProductDB implements ProductDBIF {
	...
	public Product createNewProduct(String name, double purchasePrice, double salesPrice,
			double rentPrice, String countryOfOrigin, int minStock, int productLocation, int stock) {
		...
		return new Product(identity, name, purchasePrice, salesPrice,
		 rentPrice, countryOfOrigin, minStock, productLocation, stock);
	}
	...
	public Product findProductByProductNumber(int productNumber) {
	    ...
	    try (PreparedStatement prepS = con.prepareStatement(SELECT_PRODUCT_PRODUCTNUMBER)) {
	        ...
	        if (rs != null && rs.next()) {
	            // If the result set is not null and contains a row, build a Product object from the row
	    ...
	    // Return the outputProduct variable (which will be null if no matching product was found)
	    return outputProduct;
	}

	/**
	 * Retrieves a list of all products stored at a specific location in the database.
	 *
	 * @param locationId the ID of the location to retrieve products from
	 * @return a List of Product objects stored at the specified location, or null if none were found
	 */
	@Override
	public List<Product> getProductsAtLocation(int locationId) {
	    ...
	    try (PreparedStatement prepS = con.prepareStatement(SELECT_PRODUCT_LOCATION)) {
	        ...
	        ResultSet rs = prepS.executeQuery();
	        if (rs != null && rs.next()) {
	            // If the result set is not null and contains a row, build a List of Product objects from the rows
	...
	public boolean updateProductStock(int productNumber, int stock) {
	    ...
	    // Return the success variable (which will be true if the update was successful, false otherwise)
	    return success;
	}
	...
	@return the Product object created from the ResultSet data, or null if the ResultSet is empty
	*/
